...
# ============================================
# SEND MESSAGE TO ADMIN WITH RETRY
# ============================================

async def send_to_admin(bot, message_text, max_retries=3):
    """Send message to admin with retry mechanism"""
    
    for attempt in range(max_retries):
        try:
            await bot.send_message(ADMIN_ID, message_text)
            print(f"✅ Message sent to admin (attempt {attempt + 1})")
            return True
    ...
    print(f"❌ All {max_retries} attempts failed to send to admin {ADMIN_ID}")
    return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Notify admin that bot is working
    await send_to_admin(context.bot, f"🤖 Bot started by {user_name} (ID: {user_id})")
...
async def contact_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_to_admin(context.bot, f"📞 CONTACT REQUEST\nFrom: {user.first_name} (@{user.username or 'N/A'})\nID: {user.id}")
...
async def cancel_booking_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_active_booking:
        ...
        await update.message.reply_text(f"⚠️ Cancel Booking `{booking_id}`?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
        await update.message.reply_text("❌ No active booking found!", parse_mode='Markdown')
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await query.edit_message_text("ℹ️ Massage: Day/Night\nCasual Meet Up: Day/Night\nDay Service: 1,2,4 Hours\nNight Package: 1,2,4 Hours, Full Night", parse_mode='Markdown')
    
    elif data == "menu_contact":
        user = query.from_user
        await send_to_admin(context.bot, f"📞 Contact from {user.first_name} (@{user.username or 'N/A'})")
        ...
        if user_id in user_active_booking:
            ...
            await query.edit_message_text(f"⚠️ Cancel Booking `{booking_id}`?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        else:
    ...
    # Service Selection
        ...
        keyboard = [[InlineKeyboardButton("☀️ Day", callback_data="casual_day")], [InlineKeyboardButton("🌙 Night", callback_data="casual_night")], [InlineKeyboardButton("🔙 Back", callback_data="menu_book")]]
        await query.edit_message_text("✅ Casual Meet Up\n\nSelect Day or Night:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    
    elif data == "casual_day":
        user_data[user_id]["type"] = "Day"
        user_data[user_id]["step"] = "duration"
        keyboard = [[InlineKeyboardButton("1 Hour", callback_data="dur_1")], [InlineKeyboardButton("2 Hours", callback_data="dur_2")], [InlineKeyboardButton("4 Hours", callback_data="dur_4")], [InlineKeyboardButton("🔙 Back", callback_data="book_casual")]]
        await query.edit_message_text("✅ Casual Meet Up - Day\n\nSelect duration:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    
    elif data == "casual_night":
        user_data[user_id]["type"] = "Night"
        user_data[user_id]["step"] = "duration"
        keyboard = [[InlineKeyboardButton("1 Hour", callback_data="dur_1")], [InlineKeyboardButton("2 Hours", callback_data="dur_2")], [InlineKeyboardButton("4 Hours", callback_data="dur_4")], [InlineKeyboardButton("Full Night", callback_data="dur_night")], [InlineKeyboardButton("🔙 Back", callback_data="book_casual")]]
        await query.edit_message_text("✅ Casual Meet Up - Night\n\nSelect duration:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    
    elif data == "book_day":
        user_data[user_id] = {"service": "Day Service", "step": "duration"}
        keyboard = [[InlineKeyboardButton("1 Hour", callback_data="dur_1")], [InlineKeyboardButton("2 Hours", callback_data="dur_2")], [InlineKeyboardButton("4 Hours", callback_data="dur_4")], [InlineKeyboardButton("🔙 Back", callback_data="menu_book")]]
        await query.edit_message_text("✅ Day Service\n\nSelect duration:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    
    elif data == "book_night":
        user_data[user_id] = {"service": "Night Package", "step": "duration"}
        keyboard = [[InlineKeyboardButton("1 Hour", callback_data="dur_1")], [InlineKeyboardButton("2 Hours", callback_data="dur_2")], [InlineKeyboardButton("4 Hours", callback_data="dur_4")], [InlineKeyboardButton("Full Night", callback_data="dur_night")], [InlineKeyboardButton("🔙 Back", callback_data="menu_book")]]
        ...
        await query.edit_message_text(f"✅ Duration: {duration_map[data]}\n\n📍 Where do you want the service?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        ...
        await query.edit_message_text(f"✅ Place: {place_map[data]}\n\n👥 Single or Couple?", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
...
async def complete_booking(user_id, message, contact_details):
    """Complete booking and send to admin"""
...
""", parse_mode='Markdown')
        ...
        # Send to admin - PLAIN TEXT (no markdown issues)
        admin_msg = f"""
🔔 NEW BOOKING 🔔
...
"""
        ...
        # Send to admin
        result = await send_to_admin(message.bot, admin_msg)
        
        if result:
            print(f"✅ Booking details sent to admin successfully!")
        else:
            print(f"❌ FAILED to send booking details to admin!")
            # Try one more time with even simpler message
            try:
                await message.bot.send_message(ADMIN_ID, f"NEW BOOKING! Customer: {user.first_name}, Service: {service}, ID: {booking_id}")
